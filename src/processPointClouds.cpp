    ...
    float filterRes, Eigen::Vector4f minPoint, Eigen::Vector4f maxPoint)
{
    ...
    // restrict the cloud to a region (not too far out)
    typename pcl::PointCloud<PointT>::Ptr box_cloud(new pcl::PointCloud<PointT>);

    pcl::CropBox<PointT> box(true);
    box.setMin(minPoint);
    box.setMax(maxPoint);
    box.setInputCloud(filtered_cloud);
    box.filter(*box_cloud);

    // get rid of roof points
    std::vector<int> indices;

    pcl::CropBox<PointT> roof(true);
    roof.setMin(Eigen::Vector4f(-1.5, -1.7, -1, 1));
    roof.setMax(Eigen::Vector4f(2.6, 1.7, -.4, 1));
    roof.setInputCloud(box_cloud);
    roof.filter(indices);
...
std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::SeparateClouds(
    std::unordered_set<int> inliers, typename pcl::PointCloud<PointT>::Ptr cloud) 
{
  // TODO: Create two new point clouds, one cloud with obstacles and other with segmented plane
    ...
    for (int point : inliers)
        inliers2->indices.push_back(point);
    ...
    float clusterTolerance, int minSize, int maxSize)
{
    ...
    clusterIndices = euclideanCluster(cloud, tree, clusterTolerance, minSize, maxSize);
